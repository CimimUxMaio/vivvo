defmodule VivvoWeb.UserAuth do
  ...
  @remember_me_cookie "_vivvo_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      VivvoWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      VivvoWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule VivvoWeb.PageLive do
        use VivvoWeb, :live_view

        on_mount {VivvoWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{VivvoWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_owner_role, _params, _session, socket) do
    if socket.assigns.current_scope &&
         socket.assigns.current_scope.user &&
         socket.assigns.current_scope.user.current_role == :owner do
      {:cont, socket}
    else
      socket =
        socket
